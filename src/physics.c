...
physic* create_physic(vec3 minaabb,vec3 maxaabb,unsigned char priority,float mass, float friction, float bounce, object* obj){
	...
	{ph->maxaabb[0],ph->maxaabb[1],ph->maxaabb[2]}},ph->center);
	ph->shouldmove[0]=0;
	ph->shouldmove[1]=0;
	ph->shouldmove[2]=0;
	ph->obj=obj;
...
void simulate_physic(vec3 gravity, float drag){
	...
	vec3 normal;
	for(int i=0;i<get_size_DA(physic_objects);i++){
		if(x[i]->disabled==1){
			continue;
		}
		if(x[i]->priority==1){
	...
	for(int i=0;i<get_size_DA(physic_objects);i++){
		...
		for(int i3=0;i3<iteration;i3++){
			...
			for(int i2=0;i2<get_size_DA(physic_objects);i2++){
...
void delete_physic(physic* ph){//this wont delete object
	if(ph){
...
void solve_collision_physic(physic* x, physic* y, unsigned char effect_velocity){
	...
	{x->maxaabb[0],x->maxaabb[1],x->maxaabb[2]}},x->center);
	glm_aabb_center((vec3[2]){{y->minaabb[0],y->minaabb[1],y->minaabb[2]},
	{y->maxaabb[0],y->maxaabb[1],y->maxaabb[2]}},y->center);
	vec3 overlap;
