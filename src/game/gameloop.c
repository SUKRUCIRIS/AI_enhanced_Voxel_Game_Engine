...

    {
      ...
      if (seedx != -1 || seedz != -1)
      {
        get_text_size_variadic(resss.t, 1, &width, &height, "Frame: %.2lf ms\nAverage Frame: %.2lf ms\nAverage FPS: %d\n\nSeedx: %d\nSeedz: %d\n\nJolt Body Count: %d\nJolt Active Body Count: %d\nJolt Gravity: {%.2lf | %.2lf | %.2lf}\n\nPress K to change camera\nPress F to disable/enable FXAA\nPress R to disable/enable wireframe render\n\nWhole world triangle count: %d\nCurrently rendering triangle count: %d",
                               get_frame_timems(), get_average_frame_timems(), (int)(1000.0 / get_average_frame_timems()), seedx, seedz, get_body_count_jolt(), get_active_body_count_jolt(), gravity[0], gravity[1], gravity[2], get_world_triangle_count(), get_rendered_triangle_count());
        width = 0;
        height = 1080 - height;
      ...
      else
      {
        get_text_size_variadic(resss.t, 1, &width, &height, "Frame: %.2lf ms\nAverage Frame: %.2lf ms\nAverage FPS: %d\n\nUsing heightmap texture\n\nJolt Body Count: %d\nJolt Active Body Count: %d\nJolt Gravity: {%.2lf | %.2lf | %.2lf}\n\nPress K to change camera\nPress F to disable/enable FXAA\nPress R to disable/enable wireframe render\n\nWhole world triangle count: %d\nCurrently rendering triangle count: %d",
                               get_frame_timems(), get_average_frame_timems(), (int)(1000.0 / get_average_frame_timems()), get_body_count_jolt(), get_active_body_count_jolt(), gravity[0], gravity[1], gravity[2], get_world_triangle_count(), get_rendered_triangle_count());
        width = 0;
        height = 1080 - height;
        add_text_variadic(resss.t, width, height, 1, 1, red, "Frame: %.2lf ms\nAverage Frame: %.2lf ms\nAverage FPS: %d\n\nUsing heightmap texture\n\nJolt Body Count: %d\nJolt Active Body Count: %d\nJolt Gravity: {%.2lf | %.2lf | %.2lf}\n\nPress K to change camera\nPress F to disable/enable FXAA\nPress R to disable/enable wireframe render\n\nWhole world triangle count: %d\nCurrently rendering triangle count: %d",
                          get_frame_timems(), get_average_frame_timems(), (int)(1000.0 / get_average_frame_timems()), get_body_count_jolt(), get_active_body_count_jolt(), gravity[0], gravity[1], gravity[2], get_world_triangle_count(), get_rendered_triangle_count());
